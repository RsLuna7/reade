...
├── docs/                          # WEB_DEPLOY.md、USER_GUIDE.md、roadmap-i`n`novatio`ns.md、pla`n-*.md、age`nt-skills-loop.md
├── .age`nts/                       # 按需 Skills 与 lea`ni`ngs 暂存（非 always-o`n.github/workflows/             # Pages 构建部署；build 前跑 pnpm test，不跑 Rust 检查`n
...
CI（`.github/workflows/deploy-pages.yml`）只在 push 到 main 时构建并发布 Web 站点，build 前会跑 `pnpm test`；`cargo test`、`clippy` 和 `fmt` 仍只在本地跑，不要指望 CI 兜底。n 时先跑 `pnpm test`，通过后再 `pnpm build:web` 并发布 Pages。Rust 的 `cargo test` / `clippy` / `fmt` 仍只在本地跑，不要指望 CI 兜底。
