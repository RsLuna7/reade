...
基线复核结论：§1 的全部事实仍成立（行号以当日 HEAD `3eaa31a` 为准）。一处重要发现：**HomeView 已经在挂载时一次性加载 `[0, now+1d]` 的全量会话**（供继续阅读与今日进度使用，`HomeView.tsx` L134），本卡直接复用该数据流 + `aggregateDayDocuments`（readingStats 已有的按日裁剪聚合），**零新增后端调用、零 Rust 改动**——草案 §3.2 的"两个目标日各一次窄窗口查询"不再需要。落定与修正如下：

| # | 决策 | 结论 |
|---|------|------|
| OD-D1..D3 | 均按推荐执行 | 标注优先、会话文档补位 / 精确本地日历日 / 空态整卡不渲染 |
| OD-D4 | 日历月钳制，**并修正"一年前"的算法** | 草案 §3.1 的"365 天"在跨闰日时漂移一天（如 2028-08-13 − 365d = 2027-08-14），违背"那年今日"语义。定稿统一为日历减法 `shiftMonthsClamped(now, -12 / -1)`：先锚定目标年月 1 号再取 `min(原日, 目标月天数)`，覆盖 1/31→12/31、3/31→2/28、闰年 2/29→2/28 |
| OD-D5（新） | 标注档过滤 | 仅高亮/下划线且摘录非空（书签无摘录，不参与）；tombstone（`deletedAt`）与失联路径（不在当前 documents）跳过 |
| OD-D6（新） | 取样规则 | 每组最多 3 条：标注按 `createdAt` 升序取前 3（当天时间线感，id 决平局）；不足 3 时用当日文档补位——`aggregateDayDocuments` 当日合计 ≥300s、按当日时长降序、跳过已被标注条目占用的文档（同一文档不重复出现） |
| OD-D7（新） | 摘录呈现 | 空白折叠为单空格后取前 60 个 code point，截断加"…"；来源行 = 文档标题（documents 现值）+ 类型徽标（复用 `.annotation-list-kind`，与回顾卡同款） |
| OD-D8（新） | 跳转接线 | HomeView 新增 `onOpenAnnotation?` prop，App 传 `handleSelectLibraryAnnotation`（标注中枢/回顾同一条跳转链，含重试与失联兜底）；prop 缺席（测试/降级）时回落为 `selectDocument` 打开文档不定位 |
| OD-D9（新） | 数据接线 | 标注 = HomeView 挂载时一次 `listAnnotations()`（可注入 `loadAnnotations` prop 供测试，双端同一入口：桌面 SQLite / Web IndexedDB）；会话 = 复用已加载的全量 sessions（Web 恒空数组）；两者就绪前整卡不渲染（与今日回顾卡同模式），读取失败静默降级为空 |
